...
float scene (vec3 p) {
  ...
  //d = min(d, 1.0 - abs(p.x));
  d = min(d, 1.0 - abs(p.y) - cos(4.0 * p.z * PI) * 0.1);
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  color = mix(color, vec3(0.0, 0.0, 0.0), smoothstep(0.0, 0.01, dot(d, n)));
  color*= smoothstep(0.0, 3.0, 5.0 - distance(o, p));
  color = mix(color, vec3(0.0, 0.0, 0.0), 1.0 - smoothstep(0.3, 0.5, abs(fract(dot(-d, n) * 0.5 + 0.5 + iBeat / 64.0) * 2.0 - 1.0)));
