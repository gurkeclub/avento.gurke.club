void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  color = mix(color, pow(clamp(vec3(0.0), vec3(0.5), color_fb), vec3(2.0)),  1.0 - smoothstep(-0.05, 0.15, length(color) - 0.1));

  color += (1.0 / 128.0 + 0.5 * color) * smoothstep(-2.0, 0.0, -abs(mod(fragCoord.y + iBeat * 4.0, 3.0) - 1.5));
